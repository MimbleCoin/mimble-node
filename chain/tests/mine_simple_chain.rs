...
fn spend_rewind_spend() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	clean_output_dir(".mwc_spend_rewind_spend");

	{
		let chain = init_chain(
			".mwc_spend_rewind_spend",
...
fn spend_in_fork_and_compact() {
...

	{
		...
		let tx2 = build::transaction(
			KernelFeatures::Plain { fee: 20000 },
			vec![
				build::input(consensus::MIMBLE_FIRST_GROUP_REWARD - 20000, key_id30.clone()),
				build::output(consensus::MIMBLE_FIRST_GROUP_REWARD - 40000, key_id31.clone()),
			],
			&kc,
			&pb,
		)
